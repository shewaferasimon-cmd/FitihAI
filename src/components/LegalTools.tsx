...
const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    bg: 'bg-indigo-500/10',
    placeholder: 'የንብረት መረጃ እዚህ ያስገቡ...'
  },
  {
    id: 'labor-analyzer',
    type: 'analyzer',
    title: 'የሰራተኛ ክርክር መተንተኛ',
    desc: 'የሰራተኛ እና አሰሪ መብቶችን እና ግዴታዎችን ይተንትኑ።',
    icon: ClipboardList,
    color: 'text-red-500',
    bg: 'bg-red-500/10',
    placeholder: 'የስራ ውሉን ወይም ክርክሩን እዚህ ይጥቀሱ...'
  },
  {
    id: 'family-analyzer',
    type: 'analyzer',
    title: 'የቤተሰብ ህግ መተንተኛ',
    desc: 'የጋብቻ፣ ፍቺ እና የልጅ ቀለብ ጉዳዮችን ይመርምሩ።',
    icon: Shield,
    color: 'text-rose-500',
    bg: 'bg-rose-500/10',
    placeholder: 'የቤተሰብ ጉዳዩን እዚህ ያብራሩ...'
  },
  {
    id: 'tax-analyzer',
    type: 'analyzer',
    title: 'የታክስ ተገዢነት መተንተኛ',
    desc: 'ንግድዎ ያለበትን የታክስ ግዴታዎች በኢትዮጵያ ህግ መሰረት ይለዩ።',
    icon: ClipboardList,
    color: 'text-orange-500',
    bg: 'bg-orange-500/10',
    placeholder: 'የታክስ ጥያቄዎን እዚህ ያስገቡ...'
  }
];
        ...
        {!selectedTool ? (
          ...
          >
            <div>
              ...
              <p className="text-text-muted font-bold mt-2 uppercase tracking-widest text-xs">{getSubTitle()}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTools.map((tool) => (
                <motion.button
                  ...
                  className="glass p-8 rounded-[40px] text-left border border-border-main hover:border-primary/40 transition-all group relative overflow-hidden shadow-sm"
                >
                  <div className={`w-14 h-14 rounded-2xl ${tool.bg} flex items-center justify-center ${tool.color} mb-6`}>
                    <tool.icon size={28} />
                  </div>
                  <h3 className="text-xl font-black text-text-header mb-3 flex items-center gap-2">
                    {tool.title}
                    <ChevronRight size={18} className="text-primary opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                  </h3>
                  <p className="text-text-main font-semibold text-sm leading-relaxed">{tool.desc}</p>
                </motion.button>
              ))}
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-6">
              <h3 className="text-3xl font-black text-text-header">የትኛውን መመሪያ ይፈልጋሉ?</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedTool?.options?.map(opt => (
                  <button
                    key={opt}
                    onClick={() => setSelectedOption(opt)}
                    className="glass p-6 rounded-3xl text-left border border-border-main hover:border-primary/40 hover:bg-primary/5 transition-all group flex items-center justify-between"
                  >
                    <span className="text-text-header font-bold">{opt}</span>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className={`px-4 py-2 rounded-full ${selectedTool?.bg} ${selectedTool?.color} text-xs font-black uppercase tracking-widest border border-border-main`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div className="glass p-8 rounded-[40px] border border-border-main space-y-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 text-text-header font-black">
                    ...
                    <button 
                      ...
                      className="p-3 bg-bg-glass hover:bg-primary/20 text-text-muted hover:text-text-header rounded-2xl transition-all flex items-center gap-2 text-xs font-black uppercase tracking-widest border border-border-main"
                    >
                      <Paperclip size={16} />
                      ያያይዙ (Attach)
                    </button>
                  ...
                  {attachedFiles.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                       {attachedFiles.map((file, i) => (
                         <div key={i} className="glass pl-4 pr-2 py-2 rounded-xl border border-border-main flex items-center gap-2 group">
                            {file.type.startsWith('image/') ? <ImageIcon size={14} className="text-primary" /> : <FileText size={14} className="text-blue-400" />}
                            <span className="text-xs font-bold text-text-main max-w-[100px] truncate">{file.name}</span>
                            <button 
                  ...
                  <textarea
                    ...
                    className="w-full h-64 bg-bg-sidebar/50 border border-border-main rounded-3xl p-6 text-text-header placeholder-text-muted/30 focus:ring-2 focus:ring-primary/40 focus:outline-none font-bold resize-none custom-scrollbar"
                  />
                  <button
                    ...
                    className={`
                      ...
                      ${(inputData.trim() || attachedFiles.length > 0 || selectedOption) && !isAnalyzing 
                        ...
                        : 'bg-bg-glass text-text-muted border border-border-main'}
                    `}
                  >
                ...
                <div className="glass p-6 rounded-3xl border border-border-main flex gap-4">
                  <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center text-amber-500 flex-shrink-0">
                    <Info size={20} />
                  </div>
                  <p className="text-sm text-text-muted font-bold leading-relaxed italic">
                    ማሳሰቢያ፦ ይህ መረጃ በAI የታገዘ በመሆኑ ለውሳኔዎችዎ መነሻ እንጂ ፍጹም የህግ መመሪያ አይደለም። ለተሟላ መረጃ ጠበቃ ያማክሩ።
                  </p>
                </div>
              </div>

              <div className="glass rounded-[40px] border border-border-main flex flex-col min-h-[500px] overflow-hidden shadow-sm">
                <div className="p-8 border-b border-border-main flex items-center justify-between">
                  <span className="text-xs font-black uppercase tracking-widest text-text-muted">ምላሽ (Response)</span>
                  {result && (
                    <div className="flex gap-2">
                       <span className="w-2 h-2 rounded-full bg-emerald-500" />
                       <span className="text-xs font-black text-emerald-500 uppercase tracking-widest">ተጠናቋል</span>
                    </div>
                  )}
                </div>
                
                <div className="flex-1 p-8 overflow-y-auto custom-scrollbar prose prose-slate dark:prose-invert prose-sm max-w-none">
                  {isAnalyzing ? (
                    <div className="h-full flex flex-col items-center justify-center gap-6 text-text-muted">
                    ...
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="text-text-main text-base leading-relaxed"
                    >
